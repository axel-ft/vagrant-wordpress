...
iptables -A INPUT -i ${guest_interface_name} -p tcp -m iprange --src-range ${range_ip_base}${nginx_ip_start}-${range_ip_base}${nginx_ip_end} --dport mysql -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT   # Allow New connections from nginx web servers to database
iptables -A INPUT -i ${guest_interface_name} -p tcp -m iprange --src-range ${range_ip_base}${apache_ip_start}-${range_ip_base}${apache_ip_end} --dport mysql -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT # Allow New connections from apache web servers to database
...
iptables -A OUTPUT -o ${guest_interface_name} -p tcp -m iprange --dst-range ${range_ip_base}${nginx_ip_start}-${range_ip_base}${nginx_ip_end} --sport mysql -m conntrack --ctstate ESTABLISHED -j ACCEPT   # Allow Established connections from database to nginx web servers
iptables -A OUTPUT -o ${guest_interface_name} -p tcp -m iprange --dst-range ${range_ip_base}${apache_ip_start}-${range_ip_base}${apache_ip_end} --sport mysql -m conntrack --ctstate ESTABLISHED -j ACCEPT # Allow Established connections from database to apache web servers
