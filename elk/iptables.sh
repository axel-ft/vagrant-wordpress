...
iptables -A INPUT -i ${bridgeif_guest_name} -p tcp -s ${rsyslog_hostname} --dport 5044 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT # Allow New connections to logstash only from rsyslog machine
iptables -A INPUT -i ${bridgeif_guest_name} -p tcp -s ${rsyslog_hostname} --dport 9200 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT # Allow New connections to elasticsearch only from rsyslog machine
...
iptables -A OUTPUT -o ${bridgeif_guest_name} -p tcp -d ${rsyslog_hostname} --sport 5044 -m conntrack --ctstate ESTABLISHED -j ACCEPT   # Allow Established connections to logstash only from rsyslog machine
iptables -A OUTPUT -o ${bridgeif_guest_name} -p tcp -d ${rsyslog_hostname} --sport 9200 -m conntrack --ctstate ESTABLISHED -j ACCEPT   # Allow Established connections to elasticsearch only from rsyslog machine
iptables -A OUTPUT -o ${bridgeif_guest_name} -p tcp --sport 22 -s ${cockpit_hostname} -m conntrack --ctstate ESTABLISHED -j ACCEPT
