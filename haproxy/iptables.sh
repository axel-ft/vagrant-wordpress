...
iptables -A INPUT -i ${guest_interface_name} -p tcp -s ${centreon_hostname} --sport 80 -m conntrack --ctstate ESTABLISHED -j ACCEPT # Allow connection to Kibana
iptables -A INPUT -i ${guest_interface_name} -p tcp -s ${cockpit_hostname} --sport 9090 -m conntrack --ctstate ESTABLISHED -j ACCEPT # Allow connection to Kibana
...
iptables -A OUTPUT -o ${guest_interface_name} -p tcp -d ${centreon_hostname} --dport 80 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT # Allow connection to Kibana
iptables -A OUTPUT -o ${guest_interface_name} -p tcp -d ${cockpit_hostname} --dport 9090 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT # Allow connection to Kibana
