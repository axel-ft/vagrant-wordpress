...
###########################################################################################################################################
#        Deploying 5 VM with Vagrant : 1 proxy, 1 reverse proxy, 3  web servers (wordpress), 2 file servers and one database server       #
...
Vagrant.configure("2") do |config|
  ...
  vm_params = {
    ...
    :website_name            => "Open Source Exo 3",                # Sets the website name.
  ...
  config.vm.define vm_params[:haproxy_hostname] do |haproxy|
    ...
    haproxy.vm.provision :shell, :path => "common/sethosts.sh",         :args => [vm_params[:haproxy_hostname], 14],                :name => "Set hosts",                       :env => vm_params
    haproxy.vm.provision :shell, :path => "common/setproxy.sh",         :args => [vm_params[:squid_hostname], 28],                  :name => "Set system proxy"
    haproxy.vm.provision :shell, :path => "common/apt.sh",              :args => ["neovim wget haproxy iptables-persistent", 42],   :name => "APT operations"
    haproxy.vm.provision :shell, :path => "common/enableservices.sh",   :args => ["haproxy netfilter-persistent", 56],              :name => "Enable and start services"
    haproxy.vm.provision :shell, :path => "common/iptables.sh",         :args => 70,                                                :name => "Common firewall rules"
    haproxy.vm.provision :shell, :path => "haproxy/iptables.sh",        :args => [proto, 84],                                       :name => "HAProxy specific firewall rules", :env  => vm_params
  ...
  (vm_params[:glusterfs_ip_start]..vm_params[:glusterfs_ip_end]).each do |i|
    config.vm.define "#{vm_params[:glusterfs_hostname_base]}#{i}" do |glusterfs|
      ...
      glusterfs.vm.provision :shell, :path => "common/sethosts.sh",       :args => ["#{vm_params[:gluster_hostname_base]}#{i}", 14],                 :name => "Set hosts",                         :env => vm_params
      glusterfs.vm.provision :shell, :path => "common/setproxy.sh",       :args => [vm_params[:squid_hostname], 28],                                 :name => "Set system proxy"
      glusterfs.vm.provision :shell, :path => "common/apt.sh",            :args => ["neovim unzip wget iptables-persistent glusterfs-server", 42],   :name => "APT operations"
      glusterfs.vm.provision :shell, :path => "common/enableservices.sh", :args => ["glusterd netfilter-persistent", 56],                            :name => "Enable and start services"
      glusterfs.vm.provision :shell, :path => "common/iptables.sh",       :args => 70,                                                               :name => "Common firewall rules"
      glusterfs.vm.provision :shell, :path => "glusterfs/iptables.sh",    :args => 84,                                                               :name => "GlusterFS specific firewall rules", :env  => vm_params
  ...
  (vm_params[:nginx_ip_start]..vm_params[:nginx_ip_end]).each do |i|
    config.vm.define "#{vm_params[:nginx_hostname_base]}#{i}" do |nginx|
      ...
      nginx.vm.provision :shell, :path => "common/sethosts.sh",       :args => ["#{vm_params[:nginx_hostname_base]}#{i}\topensource.axelfloquet.fr", 11],   :name => "Set hosts",                                    :env => vm_params
      nginx.vm.provision :shell, :path => "common/setproxy.sh",       :args => [vm_params[:squid_hostname], 22],                                            :name => "Set system proxy"
      nginx.vm.provision :shell, :path => "common/apt.sh",            :args => ["nginx neovim recode unzip wget php7.2-fpm php7.2-curl php7.2-gd php7.2-intl php7.2-mbstring php7.2-soap php7.2-xml php7.2-xmlrpc php7.2-zip php7.2-mysql glusterfs-client iptables-persistent", 33], :name => "APT operations"
      nginx.vm.provision :shell, :path => "common/enableservices.sh", :args => ["nginx php7.2-fpm netfilter-persistent", 44],                               :name => "Enable and start services"
      nginx.vm.provision :shell, :path => "common/iptables.sh",       :args => 55,                                                                          :name => "Common firewall rules"
      nginx.vm.provision :shell, :path => "nginx/iptables.sh",        :args => [i, proto, 66],                                                              :name => "Nginx specific firewall rules",                :env  => vm_params
      nginx.vm.provision :shell, :path => "common/glusterfsmount.sh", :args => ["--nginx", 77],                                                             :name => "Mount GlusterFS volume",                       :env  => vm_params
      nginx.vm.provision :shell, :path => "nginx/config_#{proto}.sh", :args => 88,                                                                          :name => "Nginx configuration",                          :env  => vm_params
  ...
  (vm_params[:apache_ip_start]..vm_params[:apache_ip_end]).each do |i|
    config.vm.define "#{vm_params[:apache_hostname_base]}#{i}" do |apache|
      ...
      apache.vm.provision :shell, :path => "common/sethosts.sh",        :args => ["#{vm_params[:apache_hostname_base]}#{i}\topensource.axelfloquet.fr", 11],   :name => "Set hosts",                                    :env => vm_params
      apache.vm.provision :shell, :path => "common/setproxy.sh",        :args => [vm_params[:squid_hostname], 22],                                             :name => "Set system proxy"
      apache.vm.provision :shell, :path => "common/apt.sh",             :args => ["apache2 neovim recode unzip wget php7.2-fpm php7.2-curl php7.2-gd php7.2-intl php7.2-mbstring php7.2-soap php7.2-xml php7.2-xmlrpc php7.2-zip php7.2-mysql glusterfs-client iptables-persistent", 33], :name => "APT operations"
      apache.vm.provision :shell, :path => "common/enableservices.sh",  :args => ["apache2 php7.2-fpm netfilter-persistent", 44],                              :name => "Enable and start services"
      apache.vm.provision :shell, :path => "common/iptables.sh",        :args => 55,                                                                           :name => "Common firewall rules"
      apache.vm.provision :shell, :path => "apache/iptables.sh",        :args => [i, proto, 66],                                                               :name => "Apache specific firewall rules",               :env  => vm_params
      apache.vm.provision :shell, :path => "common/glusterfsmount.sh",  :args => ["--apache", 77],                                                             :name => "Mount GlusterFS volume",                       :env  => vm_params
      apache.vm.provision :shell, :path => "apache/config_#{proto}.sh", :args => 88,                                                                           :name => "Apache configuration",                         :env  => vm_params
