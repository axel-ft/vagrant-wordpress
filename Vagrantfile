...
Vagrant.configure("2") do |config|
  ...
  vm_params = {
    ...
    :openvpn_ip             => "10.20.0.12",                        # IP for the SSH OpenVPN server allowing remote access
  ...
  config.vm.define vm_params[:cockpit_hostname] do |cockpit|
    ...
    cockpit.vm.provision :shell, run: "always", inline: "ip route delete default 2>&1 >/dev/null || true; ip route add default via 10.20.0.254"
  ...
  config.vm.define vm_params[:openvpn_hostname] do |openvpn|
    ...
    openvpn.vm.provision :shell, run: "always", inline: "ip route delete default 2>&1 >/dev/null || true; ip route add default via 10.20.0.254"
